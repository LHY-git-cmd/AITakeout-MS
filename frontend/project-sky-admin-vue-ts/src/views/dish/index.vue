...
export default class extends Vue {
  ...
  private resolveDishImage(row: any) {
    const legacyOssHost = 'sky-itcast.oss-cn-beijing.aliyuncs.com'
    if (!row.image || !row.image.includes(legacyOssHost)) {
      return row.image
    }

    const imageNumber = row.id >= 46 && row.id <= 67
      ? row.id - 45
      : ({ 68: 26, 69: 27 } as Record<number, number>)[row.id]

    return imageNumber ? `/dish-images/${row.id}.png` : row.image
  }
