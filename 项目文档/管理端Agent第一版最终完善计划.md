...
- [ ] 先编写失败测试：连续事件按序拼接，并保存最大 `seqNo`。
- [ ] 先编写失败测试：重复 `seqNo` 不得重复追加 token 或创建确认卡片。
- [ ] 先编写失败测试：连接中断后携带正确 `Last-Event-ID` 重连。
- [ ] 先编写失败测试：三次重连耗尽后查询任务终态。
- [ ] 先编写失败测试：主动取消不触发自动重连。
- [ ] 实现独立 SSE 消费模块，再将页面现有解析逻辑迁移到该模块。
- [ ] 运行 `yarn test:unit --runInBand`，预期新增测试与现有测试全部通过。
- [ ] 运行 `yarn build`，预期生产构建成功，仅允许已登记的历史警告。
...
- [ ] 编写 `normalizeAgentError(error)` 的表驱动失败测试。
- [ ] 实现错误分类模块，不以中文字符串作为唯一判断依据；优先使用 HTTP 状态和结构化错误字段。
- [ ] 页面保留已经接收的 token，不因后续连接错误覆盖已有内容。
- [ ] 为可恢复错误展示明确动作，为不可重试错误禁用重复提交提示。
- [ ] 运行前端单元测试和生产构建。
...
- [ ] 先增加其他管理员访问任务、会话、知识库和文档的 HTTP 403 测试。
- [ ] 运行指定测试并确认旧代码至少有一个场景失败。
- [ ] 将对应通用业务异常替换为权限异常，不扩大到非 Agent 业务。
- [ ] 运行 Java Agent 测试和 Maven 全量测试。
